...
class Monitor:
    def __init__(self, checkpoint, hosting_folder=None):
        ...
        os.makedirs(os.path.join(self.checkpoint, 'metric_plots'), exist_ok=True)
        ...
        os.makedirs(os.path.join(self.hosting_folder, 'metric_plots'), exist_ok=True)
...
class Metrics:
    ...
    def plot_metrics(self, epoch):
        ...
        plotly_path = os.path.join(self.checkpoint, 'metric_plots', f"{epoch}_plotly.html")
        py.plot(fig, filename=plotly_path, auto_open=False)
        if self.hosting_folder is not None:
            hosted_plotly_path = os.path.join(self.hosting_folder,'metric_plots', f"{epoch}_plotly.html")
